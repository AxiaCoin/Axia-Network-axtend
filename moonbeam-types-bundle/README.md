...
Exports npm package `moonbeam-types-bundle`, formated as per axiaaxc-js specification to use
...
In runtime 900, addresses are represented in lower case by AxiaaxcJs SDK (this should be fixed
in runtime 1000).  
However it is possible to manually encode the address in Ethereum encoded (MiXedCaSe) format using:

```
import { ethereumEncode } from "@axiaaxc/util-crypto";
