...
use axiaaxc_allychain::primitives::AccountIdConversion;
#[cfg(feature = "alphanet-native")]
use axiaaxc_service::AlphanetChainSpec;
use sc_cli::{
...
impl AxlibCli for RelayChainCli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		match id {
			...
			// Axiaaxc service to interpret the id.
			_ => axiaaxc_cli::Cli::from_iter([RelayChainCli::executable_name()].iter())
				.load_spec(id),
		}
	}

	fn native_runtime_version(chain_spec: &Box<dyn ChainSpec>) -> &'static RuntimeVersion {
		axiaaxc_cli::Cli::native_runtime_version(chain_spec)
	}
}
	...
	match &cli.subcommand {
		...
		Some(Subcommand::PurgeChain(cmd)) => {
			...
			runner.sync_run(|config| {
				...
				let axiaaxc_cli = RelayChainCli::new(
					&config,
				...
				let axiaaxc_config = AxlibCli::create_configuration(
					&axiaaxc_cli,
					&axiaaxc_cli,
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				let allychain_account =
					AccountIdConversion::<axiaaxc_primitives::v0::AccountId>::into_account(&id);
				...
				let axiaaxc_config =
					AxlibCli::create_configuration(&axiaaxc_cli, &axiaaxc_cli, tokio_handle)
						.map_err(|err| format!("Relay chain argument error: {}", err))?;
				...
				match &config.chain_spec {
					...
					spec if spec.is_moonriver() => service::start_node::<
						service::moonriver_runtime::RuntimeApi,
						service::MoonriverExecutor,
					>(config, axiaaxc_config, id, rpc_config)
					.await
					.map(|r| r.0)
					.map_err(Into::into),
					#[cfg(feature = "moonbeam-native")]
...
impl CliConfiguration<Self> for RelayChainCli {
	...
	fn init<C: AxlibCli>(&self) -> Result<()> {
		unreachable!("AxiaaxcCli is never initialized; qed");
	}
