...
use cumulus_primitives_allychain_inherent::{
	MockValidationDataInherentDataProvider, MockXcmConfig, AllychainInherentData,
};
...
async fn start_node_impl<RuntimeApi, Executor>(
	...
	axiaaxc_config: Configuration,
	id: axiaaxc_primitives::v0::Id,
	...
	Executor: NativeExecutionDispatch + 'static,
{
	...
	let relay_chain_full_node =
		cumulus_client_service::build_axiaaxc_full_node(axiaaxc_config, telemetry_worker_handle)
			.map_err(|e| match e {
				axiaaxc_service::Error::Sub(x) => x,
				s => format!("{}", s).into(),
			})?;
...
where
	...
	Executor: NativeExecutionDispatch + 'static,
{
	start_node_impl(allychain_config, axiaaxc_config, id, rpc_config).await
}
