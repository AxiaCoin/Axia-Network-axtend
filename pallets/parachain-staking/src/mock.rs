...
//! Test utilities
use crate as allychain_staking;
...
impl ExtBuilder {
	...
	pub(crate) fn build(self) -> sp_io::TestExternalities {
		...
		allychain_staking::GenesisConfig::<Test> {
...
pub(crate) fn events() -> Vec<pallet::Event<Test>> {
	System::events()
		...
		.map(|r| r.event)
		.filter_map(|e| {
			if let Event::AllychainStaking(inner) = e {
				Some(inner)
			} else {
				None
			}
...
///
/// Note that events are filtered to only match allychain-staking (see events()).
#[macro_export]
...
fn geneses() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert!(AllychainStaking::is_candidate(&1));
			assert_eq!(Balances::reserved_balance(&2), 200);
			assert_eq!(Balances::free_balance(&2), 100);
			assert!(AllychainStaking::is_candidate(&2));
			// delegators
			for x in 3..7 {
				assert!(AllychainStaking::is_delegator(&x));
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// collators
			for x in 1..5 {
				assert!(AllychainStaking::is_candidate(&x));
				assert_eq!(Balances::free_balance(&x), 80);
				assert_eq!(Balances::reserved_balance(&x), 20);
			}
			assert!(AllychainStaking::is_candidate(&5));
