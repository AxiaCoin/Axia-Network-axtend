...
use crate::mock::{
	...
	Event as MetaEvent, ExtBuilder, Origin, AllychainStaking, Test,
};
use crate::{
...
fn invalid_root_origin_fails() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::set_total_selected(Origin::signed(45), 6u32),
			sp_runtime::DispatchError::BadOrigin
		);
		assert_noop!(
			AllychainStaking::set_collator_commission(Origin::signed(45), Perbill::from_percent(5)),
			sp_runtime::DispatchError::BadOrigin
		);
		assert_noop!(
			AllychainStaking::set_blocks_per_round(Origin::signed(45), 3u32),
			sp_runtime::DispatchError::BadOrigin
		);
	});
}
...
fn set_total_selected_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AllychainStaking::set_blocks_per_round(Origin::root(), 6u32));
		assert_ok!(AllychainStaking::set_total_selected(Origin::root(), 6u32));
		assert_last_event!(MetaEvent::AllychainStaking(Event::TotalSelectedSet(
			5u32, 6u32
		)));
	});
}
...
fn set_total_selected_fails_if_above_blocks_per_round() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(AllychainStaking::round().length, 5); // test relies on this
		assert_noop!(
			AllychainStaking::set_total_selected(Origin::root(), 6u32),
...
fn set_total_selected_passes_if_equal_to_blocks_per_round() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(AllychainStaking::set_blocks_per_round(
			Origin::root(),
			10u32
		));
		assert_ok!(AllychainStaking::set_total_selected(Origin::root(), 10u32));
	});
}
...
fn set_total_selected_passes_if_below_blocks_per_round() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AllychainStaking::set_total_selected(Origin::root(), 9u32));
	});
}
...
fn set_blocks_per_round_fails_if_below_total_selected() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AllychainStaking::set_total_selected(Origin::root(), 15u32));
		assert_noop!(
			AllychainStaking::set_blocks_per_round(Origin::root(), 14u32),
			Error::<Test>::RoundLengthMustBeAtLeastTotalSelectedCollators,
		);
	});
}
...
fn set_blocks_per_round_passes_if_equal_to_total_selected() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AllychainStaking::set_total_selected(Origin::root(), 9u32));
		assert_ok!(AllychainStaking::set_blocks_per_round(Origin::root(), 9u32));
	});
}
...
fn set_total_selected_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(AllychainStaking::total_selected(), 5u32);
		assert_ok!(AllychainStaking::set_total_selected(Origin::root(), 6u32));
		assert_eq!(AllychainStaking::total_selected(), 6u32);
	});
}
...
fn cannot_set_total_selected_to_current_total_selected() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::set_total_selected(Origin::root(), 5u32),
...
fn cannot_set_total_selected_below_module_min() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::set_total_selected(Origin::root(), 4u32),
...
fn set_collator_commission_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(AllychainStaking::set_collator_commission(
			Origin::root(),
			Perbill::from_percent(5)
		));
		assert_last_event!(MetaEvent::AllychainStaking(Event::CollatorCommissionSet(
...
fn set_collator_commission_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(
			AllychainStaking::collator_commission(),
			Perbill::from_percent(20)
		);
		assert_ok!(AllychainStaking::set_collator_commission(
			Origin::root(),
...
fn cannot_set_collator_commission_to_current_collator_commission() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::set_collator_commission(Origin::root(), Perbill::from_percent(20)),
			Error::<Test>::NoWritingSameValue
		);
	});
}
...
fn set_blocks_per_round_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(AllychainStaking::set_blocks_per_round(Origin::root(), 6u32));
		assert_last_event!(MetaEvent::AllychainStaking(Event::BlocksPerRoundSet(
			1,
			0,
			5,
			6,
...
fn set_blocks_per_round_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(AllychainStaking::round().length, 5);
		assert_ok!(AllychainStaking::set_blocks_per_round(Origin::root(), 6u32));
		assert_eq!(AllychainStaking::round().length, 6);
	});
}
...
fn cannot_set_blocks_per_round_below_module_min() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::set_blocks_per_round(Origin::root(), 2u32),
			Error::<Test>::CannotSetBelowMin
		);
	});
}
...
fn cannot_set_blocks_per_round_to_current_blocks_per_round() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::set_blocks_per_round(Origin::root(), 5u32),
			Error::<Test>::NoWritingSameValue
		);
	});
}
...
fn round_immediately_jumps_if_current_duration_exceeds_new_blocks_per_round() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(17);
			assert_last_event!(MetaEvent::AllychainStaking(Event::NewRound(10, 2, 1, 20)));
			assert_ok!(AllychainStaking::set_blocks_per_round(Origin::root(), 5u32));
			roll_to(18);
			assert_last_event!(MetaEvent::AllychainStaking(Event::NewRound(18, 3, 1, 20)));
		});
}
...
fn invalid_monetary_origin_fails() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::set_staking_expectations(
				Origin::signed(45),
				Range {
					min: 3u32.into(),
					ideal: 4u32.into(),
		...
		assert_noop!(
			AllychainStaking::set_inflation(
				Origin::signed(45),
				Range {
		...
		assert_noop!(
			AllychainStaking::set_allychain_bond_account(Origin::signed(45), 11),
			sp_runtime::DispatchError::BadOrigin
		);
		assert_noop!(
			AllychainStaking::set_allychain_bond_reserve_percent(
				Origin::signed(45),
...
fn set_staking_event_emits_event_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		// valid call succeeds
		assert_ok!(AllychainStaking::set_staking_expectations(
			Origin::root(),
			Range {
				min: 3u128,
				ideal: 4u128,
		...
		assert_last_event!(MetaEvent::AllychainStaking(Event::StakeExpectationsSet(
			3u128, 4u128, 5u128,
		)));
	});
}
...
fn set_staking_updates_storage_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(
			AllychainStaking::inflation_config().expect,
			Range {
				min: 700,
				ideal: 700,
				max: 700
...
fn set_inflation_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AllychainStaking::set_inflation(
			Origin::root(),
			Range { min, ideal, max }
		));
		assert_last_event!(MetaEvent::AllychainStaking(Event::InflationSet(
			min,
			ideal,
			max,
...
fn set_inflation_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			AllychainStaking::inflation_config().annual,
			Range {
		...
		assert_eq!(
			AllychainStaking::inflation_config().round,
			Range {
...
fn cannot_set_same_inflation() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			AllychainStaking::set_inflation(Origin::root(), Range { min, ideal, max }),
			Error::<Test>::NoWritingSameValue
		);
	});
}
...
fn set_allychain_bond_account_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_last_event!(MetaEvent::AllychainStaking(Event::AllychainBondAccountSet(
			0, 11
		)));
	});
}
...
fn set_allychain_bond_account_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(AllychainStaking::allychain_bond_info().account, 0);
		assert_ok!(AllychainStaking::set_allychain_bond_account(
			Origin::root(),
			11
		));
		assert_eq!(AllychainStaking::allychain_bond_info().account, 11);
	});
}
...
fn set_allychain_bond_reserve_percent_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(AllychainStaking::set_allychain_bond_reserve_percent(
			Origin::root(),
			Percent::from_percent(50)
		));
		assert_last_event!(MetaEvent::AllychainStaking(
...
fn set_allychain_bond_reserve_percent_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(
			AllychainStaking::allychain_bond_info().percent,
			Percent::from_percent(30)
		);
		assert_ok!(AllychainStaking::set_allychain_bond_reserve_percent(
			Origin::root(),
...
fn join_candidates_increases_total_staked() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_eq!(AllychainStaking::total(), 0);
			assert_ok!(AllychainStaking::join_candidates(
				Origin::signed(1),
				10u128,
				0u32
			));
			assert_eq!(AllychainStaking::total(), 10);
		});
}
...
fn join_candidates_creates_candidate_state() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert!(AllychainStaking::candidate_info(1).is_none());
			assert_ok!(AllychainStaking::join_candidates(
				Origin::signed(1),
				10u128,
				0u32
			...
			let candidate_state =
				AllychainStaking::candidate_info(1).expect("just joined => exists");
...
fn join_candidates_adds_to_candidate_pool() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert!(AllychainStaking::candidate_pool().0.is_empty());
			assert_ok!(AllychainStaking::join_candidates(
				Origin::signed(1),
				10u128,
				0u32
			));
			let candidate_pool = AllychainStaking::candidate_pool();
...
fn cannot_join_candidates_if_candidate() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::join_candidates(Origin::signed(1), 11u128, 100u32),
...
fn cannot_join_candidates_if_delegator() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::join_candidates(Origin::signed(2), 10u128, 1u32),
...
fn cannot_join_candidates_without_min_bond() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::join_candidates(Origin::signed(1), 9u128, 100u32),
...
fn cannot_join_candidates_with_more_than_available_balance() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::join_candidates(Origin::signed(1), 501u128, 100u32),
...
fn insufficient_join_candidates_weight_hint_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			for i in 0..5 {
				assert_noop!(
					AllychainStaking::join_candidates(Origin::signed(6), 20, i),
...
fn leave_candidates_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_last_event!(MetaEvent::AllychainStaking(Event::CandidateScheduledExit(
				1, 1, 3
			)));
		});
}
...
fn leave_candidates_removes_candidate_from_candidate_pool() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_eq!(AllychainStaking::candidate_pool().0.len(), 1);
			assert_ok!(AllychainStaking::schedule_leave_candidates(
				Origin::signed(1),
				1u32
			));
...
fn cannot_leave_candidates_if_not_candidate() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::schedule_leave_candidates(Origin::signed(1), 1u32),
...
fn insufficient_leave_candidates_weight_hint_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			for i in 1..6 {
				assert_noop!(
					AllychainStaking::schedule_leave_candidates(Origin::signed(i), 4u32),
...
fn execute_leave_candidates_emits_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			assert_ok!(AllychainStaking::execute_leave_candidates(
				Origin::signed(1),
				1,
				0
			));
			assert_last_event!(MetaEvent::AllychainStaking(Event::CandidateLeft(1, 10, 0)));
		});
}
...
fn execute_leave_candidates_requires_correct_weight_hint() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 0..3 {
				assert_noop!(
					AllychainStaking::execute_leave_candidates(Origin::signed(1), 1, i),
...
fn execute_leave_candidates_removes_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let candidate_state =
				AllychainStaking::candidate_info(1).expect("just left => still exists");
			assert_eq!(candidate_state.bond, 10u128);
			roll_to(10);
			assert_ok!(AllychainStaking::execute_leave_candidates(
				Origin::signed(1),
			...
			assert!(AllychainStaking::candidate_state(1).is_none());
		});
}
...
fn cannot_execute_leave_candidates_before_delay() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::execute_leave_candidates(Origin::signed(3), 1, 0),
...
fn cancel_leave_candidates_emits_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::cancel_leave_candidates(
				Origin::signed(1),
				1
			));
			assert_last_event!(MetaEvent::AllychainStaking(Event::CancelledCandidateExit(
				1
			)));
		});
}
...
fn cancel_leave_candidates_updates_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let candidate =
				AllychainStaking::candidate_info(&1).expect("just cancelled leave so exists");
...
fn cancel_leave_candidates_adds_to_candidate_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(AllychainStaking::candidate_pool().0[0].owner, 1);
			assert_eq!(AllychainStaking::candidate_pool().0[0].amount, 10);
		});
}
...
fn go_offline_event_emits_correctly() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(AllychainStaking::go_offline(Origin::signed(1)));
			assert_last_event!(MetaEvent::AllychainStaking(Event::CandidateWentOffline(1)));
		});
}
...
fn go_offline_updates_candidate_state_to_idle() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let candidate_state = AllychainStaking::candidate_info(1).expect("is active candidate");
			assert_eq!(candidate_state.status, CollatorStatus::Active);
			assert_ok!(AllychainStaking::go_offline(Origin::signed(1)));
			let candidate_state =
				AllychainStaking::candidate_info(1).expect("is candidate, just offline");
...
fn cannot_go_offline_if_not_candidate() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::go_offline(Origin::signed(3)),
			Error::<Test>::CandidateDNE
		);
	});
}
...
fn cannot_go_offline_if_already_offline() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(AllychainStaking::go_offline(Origin::signed(1)));
			assert_noop!(
				AllychainStaking::go_offline(Origin::signed(1)),
...
fn go_online_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(AllychainStaking::go_offline(Origin::signed(1)));
			assert_ok!(AllychainStaking::go_online(Origin::signed(1)));
			assert_last_event!(MetaEvent::AllychainStaking(Event::CandidateBackOnline(1)));
		});
}
...
fn go_online_adds_to_candidate_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::go_online(Origin::signed(1)));
			assert_eq!(AllychainStaking::candidate_pool().0[0].owner, 1);
			assert_eq!(AllychainStaking::candidate_pool().0[0].amount, 20);
		});
}
...
fn go_online_storage_updates_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(AllychainStaking::go_offline(Origin::signed(1)));
			let candidate_state =
				AllychainStaking::candidate_info(1).expect("offline still exists");
			assert_eq!(candidate_state.status, CollatorStatus::Idle);
			assert_ok!(AllychainStaking::go_online(Origin::signed(1)));
			let candidate_state = AllychainStaking::candidate_info(1).expect("online so exists");
			assert_eq!(candidate_state.status, CollatorStatus::Active);
		});
}
...
fn cannot_go_online_if_not_candidate() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::go_online(Origin::signed(3)),
			Error::<Test>::CandidateDNE
		);
	});
}
...
fn cannot_go_online_if_already_online() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::go_online(Origin::signed(1)),
...
fn candidate_bond_more_emits_correct_event() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(AllychainStaking::candidate_bond_more(Origin::signed(1), 30));
			assert_last_event!(MetaEvent::AllychainStaking(Event::CandidateBondedMore(
				1, 30, 50
			)));
		});
}
...
fn candidate_bond_more_increases_total() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let mut total = AllychainStaking::total();
			assert_ok!(AllychainStaking::candidate_bond_more(Origin::signed(1), 30));
			total += 30;
			assert_eq!(AllychainStaking::total(), total);
		});
}
...
fn candidate_bond_more_updates_candidate_state() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let candidate_state = AllychainStaking::candidate_info(1).expect("updated => exists");
...
fn candidate_bond_more_updates_candidate_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::candidate_bond_more(Origin::signed(1), 30));
			assert_eq!(AllychainStaking::candidate_pool().0[0].owner, 1);
			assert_eq!(AllychainStaking::candidate_pool().0[0].amount, 50);
		});
}
...
fn cannot_schedule_candidate_bond_less_if_request_exists() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::schedule_candidate_bond_less(Origin::signed(1), 5),
...
fn cannot_schedule_candidate_bond_less_if_not_candidate() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::schedule_candidate_bond_less(Origin::signed(6), 50),
			Error::<Test>::CandidateDNE
		);
	});
}
...
fn cannot_schedule_candidate_bond_less_if_new_total_below_min_candidate_stk() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::schedule_candidate_bond_less(Origin::signed(1), 21),
				Error::<Test>::CandidateBondBelowMin
			);
		});
}
...
fn cannot_schedule_candidate_bond_less_if_exited_candidates() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::schedule_candidate_bond_less(Origin::signed(1), 10),
				Error::<Test>::CandidateDNE
			);
		});
}
...
fn execute_candidate_bond_less_emits_correct_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			assert_ok!(AllychainStaking::execute_candidate_bond_less(
				Origin::signed(1),
				1
			));
			assert_last_event!(MetaEvent::AllychainStaking(Event::CandidateBondedLess(
				1, 30, 20
			)));
		});
}
...
fn execute_candidate_bond_less_updates_candidate_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(AllychainStaking::candidate_pool().0[0].owner, 1);
			assert_eq!(AllychainStaking::candidate_pool().0[0].amount, 30);
			assert_ok!(AllychainStaking::schedule_candidate_bond_less(
				Origin::signed(1),
				10
			));
...
fn cancel_candidate_bond_less_emits_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::cancel_candidate_bond_less(
				Origin::signed(1)
			));
			assert_last_event!(MetaEvent::AllychainStaking(
...
fn cancel_candidate_bond_less_updates_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert!(AllychainStaking::candidate_info(&1)
				.unwrap()
				.request
				.is_none());
		});
...
fn only_candidate_can_cancel_candidate_bond_less_request() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::cancel_candidate_bond_less(Origin::signed(2)),
				Error::<Test>::CandidateDNE
			);
		});
}
...
fn delegate_event_emits_correctly() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(AllychainStaking::delegate(Origin::signed(2), 1, 10, 0, 0));
			assert_last_event!(MetaEvent::AllychainStaking(Event::Delegation(
				2,
				10,
				1,
...
fn delegate_updates_delegator_state() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert!(AllychainStaking::delegator_state(2).is_none());
			assert_ok!(AllychainStaking::delegate(Origin::signed(2), 1, 10, 0, 0));
			let delegator_state =
				AllychainStaking::delegator_state(2).expect("just delegated => exists");
...
fn delegate_updates_collator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			let candidate_state =
				AllychainStaking::candidate_info(1).expect("registered in genesis");
			assert_eq!(candidate_state.total_counted, 30);
			let top_delegations =
				AllychainStaking::top_delegations(1).expect("registered in genesis");
			assert!(top_delegations.delegations.is_empty());
			assert!(top_delegations.total.is_zero());
			assert_ok!(AllychainStaking::delegate(Origin::signed(2), 1, 10, 0, 0));
			let candidate_state =
				AllychainStaking::candidate_info(1).expect("just delegated => exists");
			assert_eq!(candidate_state.total_counted, 40);
			let top_delegations =
				AllychainStaking::top_delegations(1).expect("just delegated => exists");
...
fn can_delegate_immediately_after_other_join_candidates() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(AllychainStaking::join_candidates(Origin::signed(1), 20, 0));
			assert_ok!(AllychainStaking::delegate(Origin::signed(2), 1, 20, 0, 0));
		});
}
...
fn can_delegate_if_revoking() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::delegate(Origin::signed(2), 4, 10, 0, 2));
		});
}
...
fn cannot_delegate_if_leaving() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::delegate(Origin::signed(2), 3, 10, 0, 1),
...
fn cannot_delegate_if_candidate() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::delegate(Origin::signed(2), 1, 10, 0, 0),
				Error::<Test>::CandidateExists
			);
		});
}
...
fn cannot_delegate_if_already_delegated() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::delegate(Origin::signed(2), 1, 10, 1, 1),
...
fn cannot_delegate_more_than_max_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::delegate(Origin::signed(2), 6, 10, 0, 4),
...
fn sufficient_delegate_weight_hint_succeeds() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			let mut count = 4u32;
			for i in 7..11 {
				assert_ok!(AllychainStaking::delegate(
					Origin::signed(i),
					1,
					10,
					count,
...
fn insufficient_delegate_weight_hint_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 7..11 {
				assert_noop!(
					AllychainStaking::delegate(Origin::signed(i), 1, 10, count, 0u32),
			...
			for i in 3..11 {
				assert_noop!(
					AllychainStaking::delegate(Origin::signed(i), 2, 10, count, 0u32),
...
fn schedule_leave_delegators_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_last_event!(MetaEvent::AllychainStaking(Event::DelegatorExitScheduled(
				1, 2, 3
			)));
		});
}
...
fn cannot_schedule_leave_delegators_if_already_leaving() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::schedule_leave_delegators(Origin::signed(2)),
...
fn execute_leave_delegators_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			assert_ok!(AllychainStaking::execute_leave_delegators(
				Origin::signed(2),
				2,
				1
			));
...
fn execute_leave_delegators_decreases_total_staked() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_eq!(AllychainStaking::total(), 40);
			assert_ok!(AllychainStaking::schedule_leave_delegators(Origin::signed(
				2
			)));
			roll_to(10);
			...
			assert_eq!(AllychainStaking::total(), 30);
		});
}
...
fn execute_leave_delegators_removes_delegator_state() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert!(AllychainStaking::delegator_state(2).is_some());
			assert_ok!(AllychainStaking::schedule_leave_delegators(Origin::signed(
				2
			)));
			roll_to(10);
...
fn execute_leave_delegators_removes_delegations_from_collator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			for i in 2..6 {
				let candidate_state =
					AllychainStaking::candidate_info(i).expect("initialized in ext builder");
				assert_eq!(candidate_state.total_counted, 30);
				let top_delegations =
					AllychainStaking::top_delegations(i).expect("initialized in ext builder");
				assert_eq!(top_delegations.delegations[0].owner, 1);
				assert_eq!(top_delegations.delegations[0].amount, 10);
				assert_eq!(top_delegations.total, 10);
			}
			assert_eq!(
				AllychainStaking::delegator_state(1)
					.unwrap()
					.delegations
					.0
					.len(),
...
fn cannot_execute_leave_delegators_before_delay() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::execute_leave_delegators(Origin::signed(2), 2, 1),
...
fn insufficient_execute_leave_delegators_weight_hint_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 3..7 {
				assert_noop!(
					AllychainStaking::execute_leave_delegators(Origin::signed(i), i, 0),
...
fn cancel_leave_delegators_emits_correct_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::cancel_leave_delegators(Origin::signed(2)));
			assert_last_event!(MetaEvent::AllychainStaking(Event::DelegatorExitCancelled(
				2
			)));
		});
}
...
fn cancel_leave_delegators_updates_delegator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::cancel_leave_delegators(Origin::signed(2)));
			let delegator =
				AllychainStaking::delegator_state(&2).expect("just cancelled exit so exists");
...
fn revoke_delegation_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			assert_ok!(AllychainStaking::execute_delegation_request(
				Origin::signed(2),
				2,
				1
			));
...
fn cannot_revoke_delegation_if_not_delegator() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::schedule_revoke_delegation(Origin::signed(2), 1),
			Error::<Test>::DelegatorDNE
		);
	});
}
...
fn cannot_revoke_delegation_that_dne() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::schedule_revoke_delegation(Origin::signed(2), 3),
...
fn delegator_bond_more_reserves_balance() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::delegator_bond_more(
				Origin::signed(2),
				1,
				5
			));
...
fn delegator_bond_more_increases_total_staked() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(AllychainStaking::total(), 45);
		});
}
...
fn delegator_bond_more_updates_delegator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				AllychainStaking::delegator_state(2).expect("exists").total,
				10
			);
			assert_ok!(AllychainStaking::delegator_bond_more(
				Origin::signed(2),
...
fn delegator_bond_more_updates_candidate_state_top_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				AllychainStaking::top_delegations(1).unwrap().delegations[0].owner,
				2
			);
			assert_eq!(
				AllychainStaking::top_delegations(1).unwrap().delegations[0].amount,
				10
			);
			assert_ok!(AllychainStaking::delegator_bond_more(
				Origin::signed(2),
...
fn delegator_bond_more_updates_candidate_state_bottom_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				AllychainStaking::bottom_delegations(1)
					.expect("exists")
					.delegations[0]
					.owner,
				2
			...
			assert_last_event!(MetaEvent::AllychainStaking(Event::DelegationIncreased(
				2, 1, 5, false
			)));
			assert_eq!(
				AllychainStaking::bottom_delegations(1)
...
fn delegator_bond_less_updates_delegator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let state =
				AllychainStaking::delegator_state(&2).expect("just request bonded less so exists");
			assert_eq!(
...
fn cannot_delegator_bond_less_if_revoking() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::schedule_delegator_bond_less(Origin::signed(2), 1, 1),
...
fn cannot_delegator_bond_less_if_not_delegator() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			AllychainStaking::schedule_delegator_bond_less(Origin::signed(2), 1, 5),
			Error::<Test>::DelegatorDNE
		);
	});
}
...
fn cannot_delegator_bond_less_if_candidate_dne() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::schedule_delegator_bond_less(Origin::signed(2), 3, 5),
				Error::<Test>::DelegationDNE
			);
		});
}
...
fn cannot_delegator_bond_less_below_min_collator_stk() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::schedule_delegator_bond_less(Origin::signed(2), 1, 6),
...
fn cannot_delegator_bond_less_more_than_total_delegation() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::schedule_delegator_bond_less(Origin::signed(2), 1, 11),
				Error::<Test>::DelegatorBondBelowMin
			);
		});
}
...
fn cannot_delegator_bond_less_below_min_delegation() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				AllychainStaking::schedule_delegator_bond_less(Origin::signed(2), 1, 8),
...
fn cannot_execute_revoke_delegation_below_min_delegator_stake() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			assert_noop!(
				AllychainStaking::execute_delegation_request(Origin::signed(2), 2, 1),
				Error::<Test>::DelegatorBondBelowMin
			);
			// but delegator can cancel the request and request to leave instead:
			assert_ok!(AllychainStaking::cancel_delegation_request(
				Origin::signed(2),
				1
			));
			assert_ok!(AllychainStaking::schedule_leave_delegators(Origin::signed(
...
fn execute_revoke_delegation_adds_revocation_to_delegator_state() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert!(AllychainStaking::delegator_state(2)
				.expect("exists")
				.requests
				.requests
				.get(&1)
...
fn execute_revoke_delegation_removes_delegation_from_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				AllychainStaking::candidate_info(1)
					.expect("exists")
					.delegation_count,
				1u32
			);
...
fn can_execute_leave_candidates_if_revoking_candidate() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert!(!AllychainStaking::is_delegator(&2));
			assert_eq!(Balances::reserved_balance(&2), 0);
			assert_eq!(Balances::free_balance(&2), 10);
		});
}
...
fn delegator_bond_less_after_revoke_delegation_does_not_effect_exit() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::schedule_delegator_bond_less(Origin::signed(2), 1, 2),
				Error::<Test>::PendingDelegationRequestAlreadyExists
			);
			assert_ok!(AllychainStaking::schedule_delegator_bond_less(
				Origin::signed(2),
			...
			assert_last_event!(MetaEvent::AllychainStaking(Event::DelegationDecreased(
				2, 3, 2, true
			)));
			assert!(AllychainStaking::is_delegator(&2));
...
fn execute_delegator_bond_less_decreases_total_staked() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(AllychainStaking::total(), 35);
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			let pre_call_candidate_info =
				AllychainStaking::candidate_info(&1).expect("delegated by all so exists");
			let pre_call_top_delegations =
				AllychainStaking::top_delegations(&1).expect("delegated by all so exists");
			let pre_call_bottom_delegations =
				AllychainStaking::bottom_delegations(&1).expect("delegated by all so exists");
			assert_ok!(AllychainStaking::schedule_delegator_bond_less(
				Origin::signed(2),
				1,
				2
...
fn cancel_revoke_delegation_updates_delegator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let state = AllychainStaking::delegator_state(&2).unwrap();
			assert_eq!(
				state.requests().get(&1),
				Some(&DelegationRequest {
					collator: 1,
...
fn delegator_schedule_revocation_total() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				AllychainStaking::delegator_state(2)
					.expect("exists")
					.requests
					.less_total,
				10
			...
			assert_ok!(AllychainStaking::delegate(Origin::signed(2), 5, 10, 0, 2));
			assert_ok!(AllychainStaking::schedule_revoke_delegation(
				Origin::signed(2),
				3
			));
...
fn allychain_bond_inflation_reserve_matches_config() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				AllychainStaking::schedule_leave_delegators(Origin::signed(66)),
				Error::<Test>::DelegatorDNE
			);
			assert_ok!(AllychainStaking::schedule_leave_delegators(Origin::signed(
				6
			...
			set_author(8, 1, 100);
			assert_ok!(AllychainStaking::delegate(Origin::signed(8), 1, 10, 10, 10));
			roll_to(45);
...
fn paid_collator_commission_matches_config() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(9);
			assert_ok!(AllychainStaking::delegate(Origin::signed(5), 4, 10, 10, 10));
			assert_ok!(AllychainStaking::delegate(Origin::signed(6), 4, 10, 10, 10));
			roll_to(11);
			let mut new = vec![
...
fn collator_exit_executes_after_delay() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let info = AllychainStaking::candidate_info(&2).unwrap();
...
fn payout_distribution_to_solo_collators() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert!(AllychainStaking::awarded_pts(1, 1).is_zero());
			assert!(AllychainStaking::awarded_pts(4, 1).is_zero());
			assert!(AllychainStaking::awarded_pts(4, 2).is_zero());
			assert!(AllychainStaking::awarded_pts(6, 1).is_zero());
			assert!(AllychainStaking::awarded_pts(6, 2).is_zero());
			assert!(AllychainStaking::awarded_pts(6, 3).is_zero());
			assert!(AllychainStaking::awarded_pts(6, 4).is_zero());
			assert!(AllychainStaking::awarded_pts(6, 5).is_zero());
		});
}
...
fn multiple_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq_events!(expected.clone());
			assert_ok!(AllychainStaking::delegate(Origin::signed(6), 2, 10, 10, 10));
			assert_ok!(AllychainStaking::delegate(Origin::signed(6), 3, 10, 10, 10));
			assert_ok!(AllychainStaking::delegate(Origin::signed(6), 4, 10, 10, 10));
			roll_to(16);
			let mut new = vec![
			...
			assert_eq_events!(expected.clone());
			roll_to(21);
			assert_ok!(AllychainStaking::delegate(Origin::signed(7), 2, 80, 10, 10));
			assert_ok!(AllychainStaking::delegate(
				Origin::signed(10),
				2,
				10,
			...
			assert_eq!(AllychainStaking::delegator_state(7).unwrap().total, 90);
			assert_eq!(
				AllychainStaking::delegator_state(7)
					.unwrap()
					.delegations
					.0
					.len(),
			...
			assert_eq!(AllychainStaking::delegator_state(6).unwrap().total, 40);
			assert_eq!(
				AllychainStaking::delegator_state(6)
					.unwrap()
					.delegations
					.0
					.len(),
			...
			assert_eq!(AllychainStaking::delegator_state(7).unwrap().total, 10);
			assert_eq!(AllychainStaking::delegator_state(6).unwrap().total, 30);
			assert_eq!(
				AllychainStaking::delegator_state(7)
					.unwrap()
					.delegations
...
fn execute_leave_candidate_removes_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				AllychainStaking::delegator_state(3)
					.unwrap()
					.requests
					.revocations_count,
				0
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			fn is_candidate_pool_bond(account: u64, bond: u128) {
				let pool = AllychainStaking::candidate_pool();
...
fn only_top_collators_are_counted() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 3..11 {
				assert!(AllychainStaking::is_delegator(&i));
			}
			let collator_state = AllychainStaking::candidate_info(1).unwrap();
			// 15 + 16 + 17 + 18 + 20 = 86 (top 4 + self bond)
...
fn delegation_events_convey_correct_position() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let collator1_state = AllychainStaking::candidate_info(1).unwrap();
			...
			assert_ok!(AllychainStaking::delegate(Origin::signed(7), 1, 15, 10, 10));
...
fn hotfix_remove_delegation_requests_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::hotfix_remove_delegation_requests(
				Origin::root(),
				vec![2, 5]
			));
			assert!(AllychainStaking::delegator_state(&5).is_none());
			let fixed_delegator_state =
				AllychainStaking::delegator_state(&2).expect("inserted => exists");
...
fn hotfix_update_candidate_pool_value_updates_candidate_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::hotfix_update_candidate_pool_value(
				Origin::root(),
				vec![1, 2, 3, 4, 5, 6]
			));
...
fn split_candidate_state_kicks_extra_bottom_delegators_to_exit() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// total is 165 * 2 = 330
			<Total<Test>>::put(330);
			assert!(AllychainStaking::is_delegator(&11));
			assert!(AllychainStaking::is_delegator(&12));
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 3..7 {
				...
				assert_eq!(
					AllychainStaking::delegator_state(i).unwrap().status,
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 3..7 {
				assert!(<NominatorState2<Test>>::get(i).is_none());
				assert_eq!(
					AllychainStaking::delegator_state(i)
						.unwrap()
						.requests
						.requests
						.get(&1),
