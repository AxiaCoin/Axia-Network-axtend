...
use fp_evm::Context;
use moonbeam_relay_encoder::axiaaxc::AxiaaxcEncoder;
...
where
	...
	RelayEncoderWrapper<R, AxiaaxcEncoder>: Precompile,
	...
	R: pallet_evm::Config,
{
	fn execute(
		...
		match address {
			...
			a if a == hash(2053) => Some(RelayEncoderWrapper::<R, AxiaaxcEncoder>::execute(
				input, target_gas, context, is_static,
			)),
