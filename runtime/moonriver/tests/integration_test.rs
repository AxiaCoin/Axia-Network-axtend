...
use moonriver_runtime::{
	BaseFee, BlockWeights, CurrencyId, AxiaaxcXcm, Precompiles, XTokens, XcmTransactor,
};
...
fn verify_pallet_prefixes() {
	fn is_pallet_prefix<P: 'static>(name: &str) {
		...
		// https://github.com/axiatech/axlib/blob/master/frame/support/procedural/src/pallet/
	...
	// TODO: use StorageInfoTrait from https://github.com/axiatech/axlib/pull/9246
	// This is now available with axiaaxc-v0.9.9 dependencies
...
fn root_can_change_default_xcm_vers() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AxiaaxcXcm::force_default_xcm_version(
				root_origin(),
				Some(2)
			));
...
#[test]
fn make_sure_axiaaxc_xcm_cannot_be_called() {
	ExtBuilder::default()
		.with_balances(vec![
			(AccountId::from(ALICE), 2_000 * MOVR),
			(AccountId::from(BOB), 1_000 * MOVR),
		...
		.execute_with(|| {
			...
			.into();
			assert_noop!(
				Call::AxiaaxcXcm(pallet_xcm::Call::<Runtime>::reserve_transfer_assets {
