...
It is used to run the axiaaxc relay node (also needed to generate the relay specs for the allychain)
...
Moonbeam rely on `axiaaxc-launch` to provide a simple command to create a local network including
...
```
yarn launch --allychain moonbase-0.19.0
```
