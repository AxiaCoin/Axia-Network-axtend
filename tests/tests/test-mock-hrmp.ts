...
import { BN, u8aToHex } from "@axiaaxc/util";
...
import type { XcmVersionedXcm } from "@axiaaxc/types/lookup";

import { ParaId, XcmpMessageFormat } from "@axiaaxc/types/interfaces";
...
describeDevMoonbeam("Mock XCM - receive horizontal transfer", (context) => {
  ...
  before("Should Register an asset and set unit per sec", async function () {
    ...
    const { events: eventsRegister } = await createBlockWithExtrinsic(
      context,
      alith,
      context.axiaaxcApi.tx.sudo.sudo(
        context.axiaaxcApi.tx.assetManager.registerAsset(
          sourceLocation,
          assetMetadata,
          new BN(1),
          true
    ...
    const { events } = await createBlockWithExtrinsic(
      ...
      alith,
      context.axiaaxcApi.tx.sudo.sudo(
        context.axiaaxcApi.tx.assetManager.setAssetUnitsPerSecond(sourceLocation, 0)
      )
    );
    ...
    const registeredAsset = (
      (await context.axiaaxcApi.query.assets.asset(assetId)) as any
    ).unwrap();
  ...
  it("Should receive a horizontal transfer of 10 FOREIGNs to Alith", async function () {
    ...
    let alith_axc_balance = (
      await context.axiaaxcApi.query.assets.account(assetId, alith.address)
    ).balance.toBigInt();
...
describeDevMoonbeam("Mock XCM - receive horizontal transfer", (context) => {
  ...
  before("Should Register an asset and set unit per sec", async function () {
    ...
    const { events } = await createBlockWithExtrinsic(
      ...
      alith,
      context.axiaaxcApi.tx.sudo.sudo(
        context.axiaaxcApi.tx.assetManager.setAssetUnitsPerSecond(statemintLocation, 0)
      )
    );
    expect(events[1].method.toString()).to.eq("UnitsPerSecondChanged");
    expect(events[4].method.toString()).to.eq("ExtrinsicSuccess");
  ...
  it("Should receive a 10 Statemine tokens to Alith with old prefix", async function () {
    ...
    const xcmpFormat: XcmpMessageFormat = context.axiaaxcApi.createType(
      "XcmpMessageFormat",
      "ConcatenatedVersionedXcm"
    );
    const receivedMessage: XcmVersionedXcm = context.axiaaxcApi.createType(
      "XcmVersionedXcm",
      xcmMessage
    );
...
describeDevMoonbeam("Mock XCM - receive horizontal transfer of DEV", (context) => {
  ...
  before("Should send DEV to the allychain sovereign", async function () {
    ...
    paraId = context.axiaaxcApi.createType("ParaId", 2000);
    ...
    await createBlockWithExtrinsic(
      context,
      alith,
      context.axiaaxcApi.tx.balances.transfer(sovereignAddress, transferredBalance)
    );
    let balance = (
      (await context.axiaaxcApi.query.system.account(sovereignAddress)) as any
    ).data.free.toBigInt();
    expect(balance.toString()).to.eq(transferredBalance.toString());
  });

  it("Should receive MOVR from para Id 2000", async function () {
    let ownParaId = (await context.axiaaxcApi.query.allychainInfo.allychainId()) as any;
    // Get Pallet balances index
    const metadata = await context.axiaaxcApi.rpc.state.getMetadata();
    ...
    // for weight payment
    let randomBalance = (
      (await context.axiaaxcApi.query.system.account(random.address)) as any
    ).data.free.toBigInt();
