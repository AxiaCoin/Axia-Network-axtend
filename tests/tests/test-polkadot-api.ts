...
describeDevMoonbeam("Axiaaxc API - Transfers", (context) => {
  ...
  before("Setup: Create empty block with balance.transfer", async () => {
    ...
    await context.axiaaxcApi.tx.balances.transfer(testAccount, 123).signAndSend(genesisAccount);
    await context.createBlock();
  });
    ...
    signedBlock.block.extrinsics.forEach(({ method: { method, section } }, index) => {
      ...
      switch (index) {
        ...
        case 2:
          ...
          expect(context.axiaaxcApi.events.system.ExtrinsicSuccess.is(events[0])).to.be.true;
          break;
        ...
        case 3:
          ...
          expect(context.axiaaxcApi.events.system.NewAccount.is(events[1])).to.be.true;
          expect(context.axiaaxcApi.events.balances.Endowed.is(events[2])).to.be.true;
          expect(context.axiaaxcApi.events.balances.Transfer.is(events[3])).to.be.true;
          expect(context.axiaaxcApi.events.system.NewAccount.is(events[5])).to.be.true;
          expect(context.axiaaxcApi.events.balances.Endowed.is(events[6])).to.be.true;
          expect(context.axiaaxcApi.events.techCommitteeCollective.Proposed.is(events[7])).to.be;
          expect(context.axiaaxcApi.events.system.ExtrinsicSuccess.is(events[8])).to.be.true;
          break;
        default:
