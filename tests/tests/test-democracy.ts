...
describeDevMoonbeam("Democracy - genesis and preimage", (context) => {
  ...
  it("should check initial state - no referendum", async function () {
    // referendumCount
    const referendumCount = await context.axiaaxcApi.query.democracy.referendumCount();
    expect(referendumCount.toHuman()).to.equal("0");
  });
  it("should check initial state - 0x0 AllychainBondAccount", async function () {
    // referendumCount
    const allychainBondInfo = await context.axiaaxcApi.query.allychainStaking.allychainBondInfo();
  ...
  it("notePreimage", async function () {
    ...
    const encodedHash = await notePreimage(
      context,
      context.axiaaxcApi.tx.allychainStaking.setAllychainBondAccount(GENESIS_ACCOUNT),
      genesisAccount
    );

    const preimageStatus = (
      (await context.axiaaxcApi.query.democracy.preimages(encodedHash)) as any
    ).unwrap();
...
describeDevMoonbeam("Democracy - propose", (context) => {
  ...
  it("propose", async function () {
    ...
    // publicProps
    const publicProps = await context.axiaaxcApi.query.democracy.publicProps();
    // encodedHash
    expect((publicProps.toJSON() as any)[0][1]).to.equal(encodedHash);
    // prop author
    expect((publicProps.toJSON() as any)[0][2]).to.equal(GENESIS_ACCOUNT);
    // depositOf
    const depositOf = await context.axiaaxcApi.query.democracy.depositOf(0);
...
describeDevMoonbeam("Democracy - second proposal", (context) => {
  ...
  before("Setup genesis account for axlib", async () => {
    ...
    //launchPeriod
    launchPeriod = await context.axiaaxcApi.consts.democracy.launchPeriod;
    ...
    // second
    await context.axiaaxcApi.tx.democracy.second(0, 1000).signAndSend(alith);
    await context.createBlock();
  });
  ...
  it("check referendum is up", async function () {
    ...
    // referendumInfoOf
    const referendumInfoOf = await context.axiaaxcApi.query.democracy.referendumInfoOf(0);
...
describeDevMoonbeam("Democracy - vote yes on referendum", (context) => {
  ...
  before("Setup genesis account for axlib", async () => {
    ...
    // enactmentPeriod
    enactmentPeriod = await context.axiaaxcApi.consts.democracy.enactmentPeriod;
    // votingPeriod
    votingPeriod = await context.axiaaxcApi.consts.democracy.votingPeriod;
  ...
  it("vote yes", async function () {
    ...
    let allychainBondInfo = await context.axiaaxcApi.query.allychainStaking.allychainBondInfo();
...
describeDevMoonbeam("Democracy - forget notePreimage", (context) => {
  ...
  before("Setup genesis account for axlib", async () => {
    ...
    const encodedProposal =
      context.axiaaxcApi.tx.allychainStaking
  ...
  it("vote", async function () {
    ...
    const { events: eventsPropose } = await createBlockWithExtrinsic(
      context,
      genesisAccount,
      context.axiaaxcApi.tx.democracy.propose(encodedHash, PROPOSAL_AMOUNT)
    );
    ...
    const { events: eventsSecond } = await createBlockWithExtrinsic(
      context,
      alith,
      context.axiaaxcApi.tx.democracy.second(0, 1000)
    );
    ...
    const { events: eventsVote } = await createBlockWithExtrinsic(
      context,
      alith,
      context.axiaaxcApi.tx.democracy.vote(0, {
        Standard: { balance: VOTE_AMOUNT, vote: { aye: true, conviction: 1 } },
      })
    );
