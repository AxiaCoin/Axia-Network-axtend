      ...
      async () => {
        const events = await axlibTransaction(
          ...
          context.axiaaxcApi.tx.proxy.proxy(
            alith.address,
            null,
            context.axiaaxcApi.tx.balances.transfer(charleth.address, 100)
          )
        );
    ...
    await expectBalanceDifference(
      ...
      async () => {
        const events = await axlibTransaction(
          context,
          alith,
          context.axiaaxcApi.tx.proxy.addProxy(baltathar.address, "Any", 0)
        );
    ...
    await expectBalanceDifference(
      ...
      async () => {
        ...
        const events2 = await axlibTransaction(
          context,
          alith,
          context.axiaaxcApi.tx.proxy.removeProxy(baltathar.address, "Any", 0)
        );
    ...
    await expectBalanceDifference(
      ...
      async () => {
        const events = await axlibTransaction(
          context,
          alith,
          context.axiaaxcApi.tx.proxy.addProxy(baltathar.address, "Any", 2)
        );
        expect(events[7].method).to.be.eq("ExtrinsicSuccess");
    ...
    await expectBalanceDifference(
      ...
      async () => {
        const events = await axlibTransaction(
          context,
          alith,
          context.axiaaxcApi.tx.proxy.addProxy(baltathar.address, "Any", 6)
        );
        events.forEach((event) => debug(`1${event.method}(${event.data})`));
        expect(events[7].method).to.be.eq("ExtrinsicSuccess");

        const transfer = context.axiaaxcApi.tx.balances.transfer(charleth.address, 100);

        const events2 = await axlibTransaction(
          context,
          baltathar,
          context.axiaaxcApi.tx.proxy.announce(alith.address, transfer.hash)
        );
        ...
        const events3 = await axlibTransaction(
          context,
          baltathar,
          context.axiaaxcApi.tx.proxy.proxyAnnounced(
            baltathar.address,
            alith.address,
            null,
            transfer
    ...
    await expectBalanceDifference(
      ...
      async () => {
        ...
        const events2 = await axlibTransaction(
          context,
          baltathar,
          context.axiaaxcApi.tx.proxy.announce(alith.address, transfer_hash)
        );
