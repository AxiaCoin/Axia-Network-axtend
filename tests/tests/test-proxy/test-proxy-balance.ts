    ...
    await expectBalanceDifference(
      ...
      async () => {
        const events = await axlibTransaction(
          context,
          alith,
          // @ts-ignore //TODO: this is because of https://github.com/axiaaxc-js/api/issues/4264
          context.axiaaxcApi.tx.proxy.addProxy(baltathar.address, "Balances", 0)
        );
      ...
      async () => {
        const events2 = await axlibTransaction(
          ...
          context.axiaaxcApi.tx.proxy.proxy(
            alith.address,
            null,
            context.axiaaxcApi.tx.authorMapping.addAssociation(BOB_AUTHOR_ID)
          )
        );
