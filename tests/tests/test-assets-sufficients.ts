...
import { blake2AsU8a, xxhashAsU8a } from "@axiaaxc/util-crypto";
import { BN, hexToU8a, bnToHex, u8aToHex } from "@axiaaxc/util";
import Keyring from "@axiaaxc/keyring";

import { randomAsHex } from "@axiaaxc/util-crypto";
...
async function mockAssetBalance(
  ...
  let assets = (
    (await context.axiaaxcApi.query.assetManager.assetIdType(assetId)) as any
  ).toJSON();
  ...
  await context.axiaaxcApi.tx.sudo
    .sudo(
      context.axiaaxcApi.tx.system.setStorage([
  ...
  (context) => {
    ...
    before("Setup contract and mock balance", async () => {
      ...
      let beforeAssetBalance = (
        (await context.axiaaxcApi.query.assets.account(assetId, ALITH)) as any
      ).balance as BN;
      await context.createBlock();
      let alithBalance = (await context.axiaaxcApi.query.assets.account(assetId, ALITH)) as any;
    ...
    it("Send MOVR and assets to an account, then drain assets, then MOVR", async function () {
      ...
      await context.axiaaxcApi.tx.assets
        .transfer(assetId, freshAccount.address, transferAmount)
        .signAndSend(alith);

      await context.createBlock();

      let freshAccountBalance = (await context.axiaaxcApi.query.assets.account(
        assetId,
      ...
      expect(
        (
          await context.axiaaxcApi.query.system.account(freshAccount.address)
        ).sufficients.toBigInt()
      ).to.eq(1n);
      // Providers should still be 0
      expect(
        (await context.axiaaxcApi.query.system.account(freshAccount.address)).providers.toBigInt()
      ).to.eq(0n);
      ...
      await context.axiaaxcApi.tx.balances
      ...
      freshAccountBalance = (await context.axiaaxcApi.query.assets.account(
        assetId,
        freshAccount.address
      )) as any;
      ...
      expect(
        (await context.axiaaxcApi.query.system.account(freshAccount.address)).data.free.toBigInt()
      ).to.eq(0n);
    });
  },
  "Legacy",
...
describeDevMoonbeam(
  ...
  (context) => {
    ...
    it("Send MOVR and assets to an account, then drain assets, dont drain MOVR", async function () {
      ...
      expect(
        (await context.axiaaxcApi.query.system.account(freshAccount.address)).consumers.toBigInt()
      ).to.eq(1n);
