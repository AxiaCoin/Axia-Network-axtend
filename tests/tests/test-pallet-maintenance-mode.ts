...
import { BN, hexToU8a } from "@axiaaxc/util";
...
describeDevMoonbeam("Pallet Maintenance Mode - normal call shouldnt work", (context) => {
  ...
  before("Try turning maintenance mode on", async () => {
    ...
    ({ events } = await createBlockWithExtrinsic(
      context,
      alith,
      context.axiaaxcApi.tx.maintenanceMode.enterMaintenanceMode()
    ));
  });
...
describeDevMoonbeam("Pallet Maintenance Mode - with sudo shouldn't work", (context) => {
  ...
  it("shouldn't succeed with sudo", async function () {
    ...
    expect((await context.axiaaxcApi.query.maintenanceMode.maintenanceMode()).toHuman()).to.equal(
      false
    );
  });
});
...
describeDevMoonbeam("Pallet Maintenance Mode - exit mode", (context) => {
  ...
  before("Try turning maintenance mode on", async () => {
    ...
    ({ events } = await execFromAllMembersOfTechCommittee(
      context,
      context.axiaaxcApi.tx.maintenanceMode.resumeNormalOperation()
    ));
  });
  ...
  (context) => {
    ...
    before("Try turning maintenance mode on", async () => {
      ...
      ({ events } = await createBlockWithExtrinsic(
        ...
        alith,
        context.axiaaxcApi.tx.sudo.sudo(
          context.axiaaxcApi.tx.evm.call(
            ALITH,
            TEST_ACCOUNT,
            "0x0",
    ...
    it("shouldn't succeed with maintenance on", async function () {
      const error = await expectError(async () => {
        await createBlockWithExtrinsic(
          context,
          genesisAccount,
          context.axiaaxcApi.tx.crowdloanRewards.claim()
        );
      });
    ...
    it("shouldn't succeed with maintenance on", async function () {
      const error = await expectError(async () => {
        await createBlockWithExtrinsic(
          context,
          baltathar,
          context.axiaaxcApi.tx.xTokens.transfer(
            "SelfReserve", //enum
  ...
  (context) => {
    ...
    it("should succeed with maintenance on", async function () {
      await createBlockWithExtrinsic(
        context,
        sudoAccount,
        context.axiaaxcApi.tx.sudo.sudo(context.axiaaxcApi.tx.xcmTransactor.register(ALITH, 0))
      );
    ...
    it("should succeedn't with maintenance on", async function () {
      const error = await expectError(async () => {
        await createBlockWithExtrinsic(
          context,
          sudoAccount,
          context.axiaaxcApi.tx.xcmTransactor.transactThroughDerivative(
            "Relay",
            0,
            "SelfReserve",
            new BN(4000000000),
