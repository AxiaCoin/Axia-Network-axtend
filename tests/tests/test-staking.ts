...
describeDevMoonbeam("Staking - Genesis", (context) => {
  it("should match collator reserved bond reserved", async function () {
    const account = await context.axiaaxcApi.query.system.account(COLLATOR_ACCOUNT);
  ...
  it("should include collator from the specs", async function () {
    const collators = await context.axiaaxcApi.query.allychainStaking.selectedCandidates();
  ...
  it("should have collator state as defined in the specs", async function () {
    const collator = await context.axiaaxcApi.query.allychainStaking.candidateInfo(
      COLLATOR_ACCOUNT
    );
  ...
  it("should have inflation matching specs", async function () {
    const inflationInfo = await context.axiaaxcApi.query.allychainStaking.inflationConfig();
    // {
    //   expect: {
    //     min: '100.0000 kUNIT',
...
describeDevMoonbeam("Staking - Join Candidates", (context) => {
  it("should successfully call joinCandidates on ETHAN", async function () {
    ...
    await context.axiaaxcApi.tx.allychainStaking
      .joinCandidates(MIN_GLMR_STAKING, 1)
      .signAndSend(ethan);
    await context.createBlock();

    let candidatesAfter = (await context.axiaaxcApi.query.allychainStaking.candidatePool()) as any;
...
describeDevMoonbeam("Staking - Join Delegators", (context) => {
  ...
  it("should have successfully delegated stake to ALITH", async function () {
    const delegatorsAfter = (
      (await context.axiaaxcApi.query.allychainStaking.delegatorState(ETHAN)) as any
    ).unwrap();
