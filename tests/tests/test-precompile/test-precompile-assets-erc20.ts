...
import { BN, hexToU8a, bnToHex, u8aToHex, stringToHex, numberToHex } from "@axiaaxc/util";
...
describeDevMoonbeamAllEthTxTypes(
  ...
  (context) => {
    ...
    it("allows to approve transfers, and allowance matches", async function () {
      ...
      let approvals = (await context.axiaaxcApi.query.assets.approvals(
        assetId,
        ALITH,
        BALTATHAR
      )) as any;
...
describeDevMoonbeamAllEthTxTypes(
  ...
  (context) => {
    ...
    it("allows to approve transfer and use transferFrom", async function () {
      ...
      approvals = (await context.axiaaxcApi.query.assets.approvals(
        assetId,
        ALITH,
        BALTATHAR
      )) as any;
      ...
      let charletBalance = (await context.axiaaxcApi.query.assets.account(
        assetId,
        CHARLETH
      )) as any;
...
describeDevMoonbeamAllEthTxTypes("Precompiles - Assets-ERC20 Wasm", (context) => {
  ...
  it("allows to approve transfer and use transferFrom through delegateCalls", async function () {
    ...
    // Charleth balance is 1000
    let charletBalance = (await context.axiaaxcApi.query.assets.account(assetId, CHARLETH)) as any;
    expect(charletBalance.balance.eq(new BN(1000))).to.equal(true);
  });
});
