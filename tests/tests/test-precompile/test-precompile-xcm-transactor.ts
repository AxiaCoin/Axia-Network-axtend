...
describeDevMoonbeamAllEthTxTypes("Precompiles - xcm transactor", (context) => {
  ...
  before("Setup genesis account and relay accounts", async () => {
    ...
    await context.axiaaxcApi.tx.sudo
      .sudo(
        context.axiaaxcApi.tx.xcmTransactor.setTransactInfo(
  ...
  it("allows to issue transfer xcm transactor", async function () {
    ...
    const assetId = context.axiaaxcApi.createType(
      "u128",
    ...
    let beforeAssetDetails = (await context.axiaaxcApi.query.assets.asset(assetId)) as any;
    ...
    let AfterAssetDetails = (await context.axiaaxcApi.query.assets.asset(assetId)) as any;
