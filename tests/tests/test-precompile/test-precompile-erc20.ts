...
async function getBalance(context, blockHeight, address) {
  const blockHash = await context.axiaaxcApi.rpc.chain.getBlockHash(blockHeight);
  const account = await context.axiaaxcApi.query.system.account.at(blockHash, address);
...
describeDevMoonbeamAllEthTxTypes("Precompiles - ERC20 Native", (context) => {
  ...
  it("allows to call totalSupply", async function () {
    ...
    const amount = await context.axiaaxcApi.query.balances.totalIssuance();
