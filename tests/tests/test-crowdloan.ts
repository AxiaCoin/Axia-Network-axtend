...
import { stringToU8a } from "@axiaaxc/util";
import type { SubmittableExtrinsic } from "@axiaaxc/api/promise/types";
import { blake2AsHex, randomAsHex } from "@axiaaxc/util-crypto";
...
export const getAccountPayable = async (
  ...
  const accountsPayable = (await context.axiaaxcApi.query.crowdloanRewards.accountsPayable(
    address
  )) as any;
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to register the genesis account for reward", async function () {
    ...
    await context.axiaaxcApi.tx.sudo
      .sudo(
        context.axiaaxcApi.tx.crowdloanRewards.initializeRewardVec([
    ...
    let initBlock = (await context.axiaaxcApi.query.crowdloanRewards.initRelayBlock()) as any;

    // Complete initialization
    await context.axiaaxcApi.tx.sudo
      .sudo(
        context.axiaaxcApi.tx.crowdloanRewards.completeInitialization(
    ...
    let isInitialized = await context.axiaaxcApi.query.crowdloanRewards.initialized();
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to make a first claim", async function () {
    ...
    const transfer = context.axiaaxcApi.tx.crowdloanRewards.claim();
    await transfer.signAndSend(genesisAccount);
    const details = await context.axiaaxcApi.rpc.payment.queryFeeDetails(transfer.toHex());
    const claimFee =
    ...
    const account = await context.axiaaxcApi.query.system.account(GENESIS_ACCOUNT);
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should show me the money after 5 blocks, after first claim was called", async function () {
    ...
    await context.axiaaxcApi.tx.crowdloanRewards.claim().signAndSend(genesisAccount);
    await context.createBlock();
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to register the genesis account - with small amount", async function () {
    ...
    // claim
    await context.axiaaxcApi.tx.crowdloanRewards.claim().signAndSend(alithAccount);
    await context.createBlock();
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  before("Setup genesis account for axlib", async () => {
    numberOfAccounts = Number(
      (await context.axiaaxcApi.consts.crowdloanRewards.maxInitContributors) as any
    );
    const keyring = new Keyring({ type: "ethereum" });
    sudoAccount = await keyring.addFromUri(ALITH_PRIV_KEY, null, "ethereum");
  });
  ...
  it("should be able to register many accounts : " + numberOfAccounts, async function () {
    ...
    await context.axiaaxcApi.tx.sudo
      .sudo(context.axiaaxcApi.tx.crowdloanRewards.initializeRewardVec(largInput))
      .signAndSend(sudoAccount);
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to register many accounts - batch : " + numberOfAccounts, async function () {
    ...
    // should be able to register many accounts
    await context.axiaaxcApi.tx.utility
      .batch([
        await context.axiaaxcApi.tx.sudo.sudo(
          context.axiaaxcApi.tx.crowdloanRewards.initializeRewardVec(
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to initialize through democracy", async function () {
    ...
    const proposal = context.axiaaxcApi.tx.utility.batchAll(calls);
    ...
    await context.axiaaxcApi.tx.democracy.notePreimage(encodedProposal).signAndSend(sudoAccount);

    await context.createBlock();

    // Propose
    await context.axiaaxcApi.tx.democracy
      .propose(encodedHash, 1000n * GLMR)
      .signAndSend(sudoAccount);

    await context.createBlock();
    const publicPropCount = await context.axiaaxcApi.query.democracy.publicPropCount();

    // we only use sudo to enact the proposal
    await context.axiaaxcApi.tx.sudo
      .sudoUncheckedWeight(
        context.axiaaxcApi.tx.democracy.enactProposal(encodedHash, publicPropCount),
        1
      )
      .signAndSend(sudoAccount);
    ...
    let reward_info_unassociated = (
      (await context.axiaaxcApi.query.crowdloanRewards.unassociatedContributions(
        relayChainAddress_2
      )) as any
    ).unwrap();
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to burn the dust", async function () {
    ...
    let initBlock = (await context.axiaaxcApi.query.crowdloanRewards.initRelayBlock()) as any;
    let previousIssuance = (await context.axiaaxcApi.query.balances.totalIssuance()) as any;
    ...
    let issuance = (await context.axiaaxcApi.query.balances.totalIssuance()) as any;
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to associate identity", async function () {
    ...
    await context.axiaaxcApi.tx.crowdloanRewards
    ...
    await context.axiaaxcApi.tx.crowdloanRewards.claim().signAndSend(toAssociateAccount);
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to call crowdloan rewards with non-transfer proxy", async function () {
    ...
    // CreateProxy
    await context.axiaaxcApi.tx.proxy
    ...
    // Claim with proxy
    await context.axiaaxcApi.tx.proxy
      .proxy(genesisAccount.address, null, context.axiaaxcApi.tx.crowdloanRewards.claim())
      .signAndSend(proxy);
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should NOT be able to call non-claim extrinsic with non-transfer proxy", async function () {
    ...
    let { events } = await createBlockWithExtrinsic(
      context,
      proxy,
      context.axiaaxcApi.tx.proxy.proxy(
        genesisAccount.address,
        null,
        context.axiaaxcApi.tx.crowdloanRewards.updateRewardAddress(proxy.address)
      )
    );
