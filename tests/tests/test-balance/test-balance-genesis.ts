...
describeDevMoonbeam("Balance genesis", (context) => {
  ...
  it("should be accessible through axiaaxcJs", async function () {
    const genesisHash = await context.axiaaxcApi.rpc.chain.getBlockHash(0);
    const account = await context.axiaaxcApi.query.system.account.at(genesisHash, GENESIS_ACCOUNT);
