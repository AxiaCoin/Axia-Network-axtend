...
describeDevMoonbeam("Reducible Balance", (context) => {
  ...
  it("should show the reducible balanced when some amount is locked", async function () {
    ...
    let existentialDeposit = (await context.axiaaxcApi.consts.balances.existentialDeposit) as any;

    // Let's lock some funds by doing a public referendum proposal
    let lock_amount = (await context.axiaaxcApi.consts.democracy.minimumDeposit) as any;
    const proposal = context.axiaaxcApi.tx.balances.setBalance(TEST_ACCOUNT, 100, 100);
