...
describeDevMoonbeamAllEthTxTypes("Balance extrinsics", (context) => {
  it("should appear after transfer", async function () {
    ...
    const blockHash = await context.axiaaxcApi.rpc.chain.getBlockHash(1);
    const signedBlock = await context.axiaaxcApi.rpc.chain.getBlock(blockHash);
    ...
    signedBlock.block.extrinsics.forEach(({ method: { method, section } }, index) => {
      ...
      switch (index) {
        ...
        case 3:
          ...
          expect(context.axiaaxcApi.events.balances.Endowed.is(events[7])).to.be.true; // treasury
          expect(context.axiaaxcApi.events.treasury.Deposit.is(events[8])).to.be.true;
          expect(context.axiaaxcApi.events.ethereum.Executed.is(events[9])).to.be.true;
          expect(context.axiaaxcApi.events.system.ExtrinsicSuccess.is(events[10])).to.be.true;
          break;
        default:
