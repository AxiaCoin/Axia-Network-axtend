// As inspired by https://github.com/axiatech/txwrapper/blob/master/examples/axiaaxc.ts
...
import { EXTRINSIC_VERSION } from "@axiaaxc/types/extrinsic/v4/Extrinsic";
...
describeDevMoonbeam("Balance transfer - txwrapper", (context) => {
  ...
  it("should reflect balance identically on axiaaxc/web3", async function () {
    const block1Hash = await context.axiaaxcApi.rpc.chain.getBlockHash(1);
    expect(await context.web3.eth.getBalance(GENESIS_ACCOUNT, 1)).to.equal(
      (
        await context.axiaaxcApi.query.system.account.at(block1Hash, GENESIS_ACCOUNT)
      ).data.free.toString()
    );
  });
