...
describeDevMoonbeam("Web3Api Information", (context) => {
  it("should include client version", async function () {
    ...
    let specName: string = await context.axiaaxcApi.runtimeVersion.specName.toString();
    let specVersion: string = await context.axiaaxcApi.runtimeVersion.specVersion.toString();
    let implVersion: string = await context.axiaaxcApi.runtimeVersion.implVersion.toString();
