...
const execFromAllMembersOfTechCommittee = async <Call extends SubmittableExtrinsic<ApiTypes>>(
  ...
  const { events: proposalEvents } = await createBlockWithExtrinsicAllychain(
    allychainApi,
    key_1,
    allychainApi.tx.techCommitteeCollective.propose(2, axiaaxcCall, lengthBound)
  );
...
describeAllychain(
  ...
  (context) => {
    it("should be able to receive an asset from relay", async function () {
      ...
      const allychainOne = context.axiaaxcApiParaone;
      const relayOne = context._axiaaxcApiRelaychains[0];
...
describeAllychain("XCM - send_relay_asset_to_relay", { chain: "moonbase-local" }, (context) => {
  ...
  before("First send relay chain asset to allychain", async function () {
    ...
    relayOne = context._axiaaxcApiRelaychains[0];
    ...
    allychainOne = context.axiaaxcApiParaone;
...
describeAllychain(
  ...
  (context) => {
    ...
    before("First send relay chain asset to allychain", async function () {
      ...
      baltathar = await keyring.addFromUri(BALTATHAR_PRIV_KEY, null, "ethereum");
      allychainOne = context.axiaaxcApiParaone;
      allychainTwo = context._axiaaxcApiAllychains[1].apis[0];
...
describeAllychain(
  ...
  (context) => {
    ...
    before("First send relay chain asset to allychain", async function () {
      ...
      allychainOne = context.axiaaxcApiParaone;
      allychainTwo = context._axiaaxcApiAllychains[1].apis[0];
      allychainThree = context._axiaaxcApiAllychains[2].apis[0];
