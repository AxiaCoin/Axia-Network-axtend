...
import type { BlockHash } from "@axiaaxc/types/interfaces/chain/types";
...
export interface DevTestContext {
  ...
  createAxiaaxcApi: () => Promise<ApiPromise>;
...
export function describeDevMoonbeam(
  ...
  describe(title, function () {
    ...
    before("Starting Moonbeam Test Node", async function () {
      ...
      context.createAxiaaxcApi = async () => {
        const apiPromise = await provideAxiaaxcApi(init.wsPort);
        // We keep track of the axiaaxcApis to close them at the end of the test
        context._axiaaxcApis.push(apiPromise);
      ...
      context.axiaaxcApi = await context.createAxiaaxcApi();
      ...
      context.createBlock = async <T>(options: BlockCreation = {}) => {
        ...
        const block = await createAndFinalizeBlock(context.axiaaxcApi, parentHash, finalize);
        return {
          txResults,
          block,
        };
    ...
    after(async function () {
      ...
      await Promise.all(context._axiaaxcApis.map((p) => p.disconnect()));
