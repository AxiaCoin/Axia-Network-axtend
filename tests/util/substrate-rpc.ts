...
import { AddressOrPair, ApiTypes, SubmittableExtrinsic } from "@axiaaxc/api/types";
import { GenericExtrinsic } from "@axiaaxc/types/extrinsic";
import { AnyTuple } from "@axiaaxc/types/types";
import { Event } from "@axiaaxc/types/interfaces";
import { u8aToHex } from "@axiaaxc/util";
...
>(
  ...
  const extrinsicHash = (await axiaaxcCall.signAndSend(sender)) as any;
  ...
  const allRecords = await context.axiaaxcApi.query.system.events.at(blockResult.block.hash);

  // We retrieve the block (including the extrinsics)
  const blockData = await context.axiaaxcApi.rpc.chain.getBlock(blockResult.block.hash);
...
>(
  ...
  const extrinsicHash = (await axiaaxcCall.signAndSend(sender)) as unknown as Uint8Array;
