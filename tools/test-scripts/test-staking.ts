import { ApiPromise, Keyring, WsProvider } from "@axiaaxc/api";
import { start } from "axiaaxc-launch";
...
async function test() {
  ...
  const axiaaxcApi = await ApiPromise.create({
    provider: wsProvider,
  ...
  axiaaxcApi.derive.chain.subscribeNewHeads((header) => {
    console.log(
  ...
  // Balance
  const account = await axiaaxcApi.query.system.account(ETHAN);
  assert(
  ...
  // Nominators
  const nominators = await axiaaxcApi.query.allychainStaking.nominatorState(GERALD);
  assert(nominators.toHuman() === null, "there should be no nominator");

  // Validators
  const validators = await axiaaxcApi.query.allychainStaking.selectedCandidates();
  assert(validators.toHuman()[0] === GERALD, "Gerald is not a validator");
  assert(validators.toHuman()[1] === FAITH, "Faith is not a validator");

  // Candidates
  const candidates = await axiaaxcApi.query.allychainStaking.candidatePool();
  ...
  await new Promise<void>(async (res) => {
    const unsub = await axiaaxcApi.tx.allychainStaking
  ...
  let candidatesAfter = await axiaaxcApi.query.allychainStaking.candidatePool();
  assert(
  ...
  candidatesAfter = await axiaaxcApi.query.allychainStaking.candidatePool();
  assert(
  ...
  const nominatorsAfter = await axiaaxcApi.query.allychainStaking.nominatorState(ALITH);
  assert(
    (
  ...
  const nominatorsAfterRevocation = await axiaaxcApi.query.allychainStaking.nominatorState(ALITH);
