...
import { killAll, run } from "axiaaxc-launch";
...
const allychains: { [name: string]: AllychainConfig } = {
  ...
  "moonbase-0.19.0": {
    relay: "betanet-9130",
    chain: "moonbase-local",
    docker: "purestake/moonbeam:v0.19.1",
  },
  local: {
    relay: "betanet-9111",
    chain: "moonbase-local",
...
async function start() {
  ...
  if (relay.binary) {
    ...
    relayBinary = `build/${relayName}/axiaaxc`;
    const relayPath = path.join(__dirname, `build/${relayName}/axiaaxc`);
    if (!fs.existsSync(relayPath)) {
      console.log(`     Missing ${relayBinary} locally, downloading it...`);
      child_process.execSync(`mkdir -p ${path.dirname(relayPath)} && \
          docker create --name axiaaxc-tmp ${relay.docker} && \
          docker cp axiaaxc-tmp:/usr/local/bin/axiaaxc ${relayPath} && \
          docker rm axiaaxc-tmp`);
  ...
  const knownRelayChains = ["axctest", "alphanet", "betanet", "axiaaxc"]
