...
Based on [axiaaxc-launch](https://github.com/axiatech/axiaaxc-launch), the tool to launch
...
# relay
local: {
  binary: "../../axiaaxc/target/release/axiaaxc",
  chain: "betanet-local",
},
```
...
Options:
  ...
  --relay-chain      overrides relay chain/runtime                      [string]
                     [choices: "betanet", "alphanet", "axctest", "axiaaxc",
...
| ------------------------ | ------------------------------------- |
| `cargo moonbase`         | `moonbase, alphanet, axiaaxc`         |
| `cargo moonbase-betanet`  | `moonbase, betanet, alphanet, axiaaxc` |
| `cargo moonriver`        | `moonriver, axiaaxc`                 |
| `cargo moonriver-betanet` | `moonriver, betanet, axiaaxc`         |
| `cargo moonriver-axctest` | `moonriver, axctest, axiaaxc`         |
| `cargo moonbeam`         | `moonbeam, axiaaxc`                  |
| `cargo moonbeam-betanet`  | `moonbeam, betanet, axiaaxc`          |

- The `moonbase` native runtime require `alphanet` native runtime to compile.
- The `axiaaxc` native runtime is always included (This is requirement from axiaaxc repo).
...
For the default configuration, you can access through axiaaxcjs:

- relay node 1: https://axiaaxc.js.org/apps/?rpc=ws://localhost:34002
- allychain node 1: https://axiaaxc.js.org/apps/?rpc=ws://localhost:34102
...
🚀 Allychain: moonriver-genesis-fast   - purestake/moonbase-allychain:moonriver-genesis-fast (moonriver-local)
     Missing build/axctest-9030-fast/axiaaxc locally, downloading it...
     build/axctest-9030-fast/axiaaxc downloaded !
...
```
npm run list-pull-request-labels -- --from axiaaxc-v0.9.4 --to axiaaxc-v0.9.5 --repo axiatech/axlib
```
...
Options:
  ...
  --repo        which repository to read                     [string] [required]
                [choices: "axiatech/axlib", "axiatech/axiaaxc"]
...
```
> npm run list-pr-labels -- --from axiaaxc-v0.9.4 --to axiaaxc-v0.9.5 --repo axiatech/axlib --only-label runtime

found 55 total commits in https://github.com/axiatech/axlib/compare/axiaaxc-v0.9.4...axiaaxc-v0.9.5
===== E1-runtimemigration
