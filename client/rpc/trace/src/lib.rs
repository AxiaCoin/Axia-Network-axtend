	...
	C::Api: ApiExt<B>,
{
	...
	fn cache_block(
		...
		let api_version = if let Ok(Some(api_version)) =
			api.api_version::<dyn EthereumRuntimeRPCApi<B>>(&axlib_block_id)
		{
			api_version
		} else {
		...
		let (eth_block, eth_transactions) = if api_version < 2 {
			...
			let (eth_block, _, eth_transactions) = api
				.current_all_before_version_2(&axlib_block_id)
				.map_err(|e| {
					internal_err(format!(
					"Failed to get Ethereum block data for Axlib block {} version < 2: {:?}",
					axlib_hash, e
				))
				})?;
			...
			#[allow(deprecated)]
			let (eth_block, _, eth_transactions) = api
				.current_all_before_version_4(&axlib_block_id)
				.map_err(|e| {
					internal_err(format!(
					"Failed to get Ethereum block data for Axlib block {} version < 4: {:?}",
					axlib_hash, e
				))
				})?;
			...
			let (eth_block, _, eth_transactions) =
				api.current_all(&axlib_block_id).map_err(|e| {
					internal_err(format!(
						"Failed to get Ethereum block data for Axlib block {} : {:?}",
						axlib_hash, e
					))
